...
func (m *boardPanelModel) rowsVisible() int {
	...
	// shared header(5: tab bar, stats, digest x2, divider) + gap(1) + footer; search costs blank+line(2)
	budget := h - 6 - lipgloss.Height(m.renderFooter(newPaletteStyles(), w))
	if m.searching {
		budget -= 2
	}
	if budget < 3 {
...
func (m *boardPanelModel) render(styles paletteStyles, width, height int) string {
	...
	hdr = append(hdr, m.tl.dividerLine(false))
