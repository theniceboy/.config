...
func runClipboardSync(args []string) error {
	...
	switch direction {
	case "pull":
		return clipboardSyncPull(host)
	case "push":
		return clipboardSyncPush(host)
	default:
		return fmt.Errorf("unknown direction %q (want pull or push)", direction)
	}
}

func clipboardSyncPull(host string) error {
	...
	fmt.Printf("pulled %s from %s\n", clipboardSizeLabel(len(remote)), host)
	return nil
}

func clipboardSyncPush(host string) error {
	...
	fmt.Printf("pushed %s to %s\n", clipboardSizeLabel(len(local)), host)
	return nil
}
